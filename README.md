...
[1\.2](#pillarUSERS)\)\. The Pillar screenshot below \(see Figure 
...
[6](#commandLineInterface) for more information about the command\-line interface\):
...
[4](#configuring) for more information about the configuration\)\. Create your first 
`pillar.conf` file:
...
[5](#templating) for more information about templating\)\. You must write such template in its own file, e\.g\., 
`myhtml.template`:
...
generates Figure 
[3\.1](#pierLogo) \(this reference has been generated using 
`*pierLogo*`\)\.
...
`*script1*` \(produces 
[3\.1](#script1)\)\. Specifying the language \(here 
